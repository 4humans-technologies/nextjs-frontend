...
import {
  ...
  CoverUpdate,
  ProfileUpdate,
} from "../UI/Profile/UserProfileUpdate"
...
function UserProfile() {
  ...
  if (authContext.user.user) {
    ...
    backgroundImage = authContext.user.user?.relatedUser?.backgroundImage
  }

  const profileEditHandler = async () => {
    const fet = await fetch("/api/website/profile/viewer/update-profile-info", {
      method: "PUT",
      headers: {
    ...
    <div className="tw-bg-dark-background ">
      ...
      <div className="tw-w-full tw-relative  tw-bg-dark-background tw-mt-[4.5rem]">
        ...
        <p
          ...
          onClick={() =>
            modelCtx.showModalWithContent(<CoverUpdate />, {
              contentStyles: {
      ...
      <div className="tw-w-full tw-bg-first-color tw-h-28 tw-flex tw-pl-8 tw-relative">
        ...
        {authContext.user.user.username ==
          ...
          <CreateIcon
            ...
            fontSize="medium"
            onClick={() =>
              modelCtx.showModalWithContent(<ProfileUpdate />, {
                contentStyles: {
                  minWidth: "min(400px, 90%)",
                },
              })
      ...
      <div className="tw-grid md:tw-grid-cols-7 tw-grid-cols-1 md:tw-gap-4   md:tw-py-2 hover:tw-shadow-lg tw-rounded-t-xl tw-rounded-b-xl tw-text-white tw-w-full tw-bg-dark-background ">
        <div className="md:tw-col-span-4 tw-col-span-1 tw-grid tw-grid-cols-4 tw-mx-2 ">
          <div className="md:tw-col-span-4 tw-col-span-4 tw-grid tw-grid-cols-4 tw-bg-first-color  tw-py-4 md:tw-my-0 tw-mt-4 tw-mb-4 tw-rounded-t-lg tw-rounded-b-lg tw-pl-4">
            ...
            <div className="md:tw-col-span-3 tw-col-span-2 ">
              ...
              >
                {authContext.user.user.relatedUser?.hobbies.map((item) => item)}
              </p>
        ...
        <div className="tw-bg-first-color md:tw-col-span-3 tw-col-span-1 md:tw-my-0 tw-my-4 tw-rounded-t-lg tw-rounded-b-lg tw-justify-items-start tw-mx-2">
          <h1 className="tw-pl-4 tw-pt-4">Friends</h1>
          ...
          <div className="tw-max-h-96   tw-text-white tw-overflow-y-auto">
            {followerData ? (
              ...
              <h1 className="tw-text-white tw-font-bold tw-pt-4">No Friends</h1>
            )}
          </div>
        </div>
      </div>
