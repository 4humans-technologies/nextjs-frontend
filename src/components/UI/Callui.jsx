...
function callui() {
    ...
    <div className="tw-text-white  tw-inline-block  tw-bg-gray-500  tw-text-center tw-ml-8 tw-mt-8 tw-py-12 tw-px-12 tw-rounded-t-2xl tw-rounded-b-2xl">
      ...
      <div>
        <div className="tw-pl-4 tw-mb-4 ">
          <img className="tw-rounded-full tw-w-32 tw-h-32" src="/pp.jpg"></img>
        </div>
      ...
      <div>
        {callRecived ? (
          ...
          <div className="tw-flex tw-w-44 tw-justify-between">
            ...
            <button className="reject_call" onClick={modalCtx.hideModal}>
              <PhoneDisabledIcon />
            </button>
          </div>
        )}
  ...
  return (
    <div>
      <button onClick={() => modalCtx.showModalWithContent(callCard)}>
        open
      </button>
    </div>
  );
